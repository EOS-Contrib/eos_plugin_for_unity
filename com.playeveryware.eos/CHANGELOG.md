...
- Change: Increased editor logging to VeryVerbose.
...
- Fix: Sandbox and Deployment ids provided by command line arguments now apply correctly to the PlatformConfig when the native libs create the PlatformInterface instance.
- Fix: Platform friends now show up in the overlay.
- Fix: EOS_DISABLE no longer causing build failures for non-Windows platforms.
...
- Fix: Null Reference Error in Sessions & Matchmaking Scene When Opening Friends Tab
